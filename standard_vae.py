...
if __name__ == "__main__":
    dataset = datasets.CIFAR10(root='./data', train = True, transform = transforms.Compose([transforms.ToTensor()]),download=True)
    ...
    for epoch in range(100):
        ...
        print("Total Losses : {:.5f} Reconstruction Losses : {:.5f} KL Divergence : {:.5f}".format(np.mean(losses), np.mean(recons_losses), np.mean(kld_losses)))
